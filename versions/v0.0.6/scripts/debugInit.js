...
export function initDebug() {
...

    {
        ...
        window.addEventListener("error", (e) => {
            text = `JS Error caught via event listener:<br><pre>Message: ${e.message}<br><pre>Source: ${e.filename}<br><pre>Line: ${e.lineno}<br><pre>Column: ${e.colno}<br><pre>Error object: ${e.error}<br><br>`
            log.innerHTML += text
        })
    }
}